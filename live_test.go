...
func TestLive(t *testing.T) {
	...
	t.Logf("took %v to move %.2f degrees", elapsed, 180.0)
...
func TestLive2(t *testing.T) {
	test, err := servo.Connect(15)
	if err != nil {
		t.Fatalf("Could not connect servo to pin 15, got:\n%v", err)
	}
	defer func() {
		test.Speed(0.05)
		test.MoveTo(90)
	...
	_t := time.Duration(570*2) * time.Millisecond
	const tolerance = 50 * time.Millisecond
	min := _t - tolerance
	max := _t + tolerance
